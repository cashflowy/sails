...
module.exports = function updateOneRecord (req, res) {
  ...
  if(Object.keys(queryOptions.criteria.where).indexOf(Model.primaryKey)==-1)
    return res.badRequest('primaryKey is not mentioned in the request');
  // - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  ...
  Model.findOne(
    _.cloneDeep(queryOptions.criteria),
  ...
  .exec(function (err, matchingRecord) {
    ...
    Model.updateOne(_.cloneDeep(queryOptions.criteria))
