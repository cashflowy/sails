...
module.exports = function destroyOneRecord (req, res) {
  ...
  var query = Model.findOne(_.cloneDeep(queryOptions.criteria), queryOptions.populates).meta(queryOptions.meta);
  query.exec(function foundRecord (err, record) {
    ...
    Model.destroy(_.cloneDeep(queryOptions.criteria)).meta(queryOptions.meta)
    .exec(function destroyedRecord (err) {
      ...
      if (req._sails.hooks.pubsub) {
        Model._publishDestroy(queryOptions.criteria[Model.primaryKey], !req._sails.config.blueprints.mirror && req, {previous: record});
        if (req.isSocket) {
